...
jest.mock('src/gateways/payment-gateway');
...
describe('OrderController', () => {
    ...
    beforeEach(() => {
        ...
        (OrderUseCases.updateStatusOnPaymentReceived as jest.Mock).mockResolvedValue(undefined);
    });
    ...
    describe('findAll', () => {
        it('should return a JSON string with all orders', async () => {
            ...
            (OrderAdapter.adaptArrayJson as jest.Mock).mockReturnValue(JSON.stringify(mockOrders));
    ...
    describe('findById', () => {
        it('should return the details of a specific order', async () => {
            ...
            (OrderAdapter.adaptJson as jest.Mock).mockReturnValue(JSON.stringify(mockOrder));

            const result = await OrderController.findById(database, '123');

            expect(result).toBe(JSON.stringify(mockOrder));
            expect(OrderUseCases.findById).toHaveBeenCalledWith(expect.any(OrderGateway), '123');
        });
    });
    ...
    describe('delete', () => {
        it('should delete an order', async () => {
            ...
            expect(OrderUseCases.delete).toHaveBeenCalledWith(expect.any(OrderGateway), '123');
        });
    });
    ...
    describe('update', () => {
        it('should update the status of an order', async () => {
            ...
            (OrderAdapter.adaptJson as jest.Mock).mockReturnValue(JSON.stringify(updatedOrder));

            const result = await OrderController.update(database, '123', 'AWAITING');

            expect(result).toBe(JSON.stringify(updatedOrder));
            expect(OrderUseCases.update).toHaveBeenCalledWith(expect.any(OrderGateway), '123', 'AWAITING');
        });
    });

    describe('updateStatusOnPaymentReceived', () => {
        it('should update the status when payment is received', async () => {
            await OrderController.updateStatusOnPaymentReceived(database, '123', true);
